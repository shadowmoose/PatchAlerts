...
parser = argparse.ArgumentParser(
    description="Tool for scanning Game patch notes, and relaying them to you.")
parser.add_argument(
    "--test", help="Tells the program to check all games & exit on errors.", action="store_true")
parser.add_argument(
    "--update", help="Update the config file and exit.", action="store_true")
parser.add_argument("--base_dir", '-d',
                    help="Override base storage directory.", type=str, metavar='')
parser.add_argument("--schedule", '-s',
                    help="CRON time - if provided, will keep scanning on schedule.", type=str, metavar='')
args = parser.parse_args()

if args.test:
    print('Running in test mode. All Games are enabled, and errors will crash the program.')

storage_dir = os.path.abspath(os.path.dirname(
...
def load():
    ...
    # Connect to (or build) the SQLite DB.
...
def scan(all_games):
    ...
    for s in all_games:
        ...
        try:
            ...
            if not _found:
                raise Exception(
                    'ERROR: Handler [%s] found 0 updates! Expects at least 1.' % s.name)
...
def alert(updates, alerts):
    ...
    for u in updates:
        ...
        if u.game in skip_games or (not args.test and not db.contains_game(u)):  # !cover
            p.out(
                '\tFirst time finding updates for [%s], will not alert!' % u.game)
...
def run():
    ...
    if args.test:
        ...
        print('DB Size: %s bytes' % os.path.getsize(
            os.path.join(storage_dir, 'db.sqldb')))
